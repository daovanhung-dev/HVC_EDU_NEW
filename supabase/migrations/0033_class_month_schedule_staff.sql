-- Keep the planned staff for a class-month while allowing the actual staff
-- roster to differ by weekly schedule slot.
create table if not exists public.class_month_schedule_staff (
  schedule_id uuid not null references public.class_month_schedules(id) on delete restrict,
...
create index if not exists class_month_schedule_staff_staff_idx
  on public.class_month_schedule_staff(staff_id);

alter table public.class_month_schedule_staff enable row level security;

revoke all on public.class_month_schedule_staff from anon;
grant select, insert, update, delete on public.class_month_schedule_staff to authenticated;
grant all on public.class_month_schedule_staff to service_role;

create or replace function public.can_view_class_month_schedule_staff(p_schedule_id uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select public.is_root()
    or public.has_permission('CLASS_VIEW')
    or exists (
      select 1
      from public.class_month_schedule_staff own_mapping
      join public.staff own_staff on own_staff.id = own_mapping.staff_id
      where own_mapping.schedule_id = p_schedule_id
        and own_staff.user_id = auth.uid()
        and own_staff.status = 'ACTIVE'
    );
$$;

revoke all on function public.can_view_class_month_schedule_staff(uuid) from public, anon;
grant execute on function public.can_view_class_month_schedule_staff(uuid) to authenticated, service_role;

drop policy if exists class_month_schedule_staff_select on public.class_month_schedule_staff;
create policy class_month_schedule_staff_select
  on public.class_month_schedule_staff
  for select
  using (public.can_view_class_month_schedule_staff(schedule_id));

drop policy if exists class_month_schedule_staff_manage on public.class_month_schedule_staff;
create policy class_month_schedule_staff_manage
  on public.class_month_schedule_staff
  for all
  using (public.is_root() or public.has_permission('CLASS_MONTH_MANAGE'))
  with check (public.is_root() or public.has_permission('CLASS_MONTH_MANAGE'));

-- New schedules use their explicit mapping. Existing class-months without a
-- mapping retain the previous class_month_staff fallback.
  ...
  loop
    ...
    loop
      if not exists (
        ...
        where mapping.schedule_id = v_schedule.id
        union all
        ...
        where fallback.class_month_id = p_class_month_id
          and not exists (
            select 1
            from public.class_month_schedule_staff mapped
            where mapped.schedule_id = v_schedule.id
          );
      end if;
    end loop;
  end loop;
