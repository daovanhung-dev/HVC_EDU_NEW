...
function valueFrom(record: Record<string, unknown> | undefined, key: string): string {
  return text(record?.[key])
}

async function ensureAvailable(admin: AdminClient, table: 'profiles' | 'staff' | 'students', column: string, value: string, duplicateCode: string) {
  if (!value) return
...
async function rollbackCreatedAccount(admin: AdminClient, userId: string) {
  ...
  if (failures.length) throw appError('ACCOUNT_ROLLBACK_FAILED', failures.join('; '))
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!rawBody || typeof rawBody !== 'object' || Array.isArray(rawBody)) throw appError('INVALID_INPUT')
    ...
    const displayName = text(body.display_name) || valueFrom(body.student, 'full_name') || valueFrom(body.staff, 'full_name')
    if (!displayName) throw appError('INVALID_INPUT')
    const phone = text(body.phone)
    const emailInput = text(body.email)
    if (emailInput && !/^\S+@\S+\.\S+$/.test(emailInput)) throw appError('INVALID_EMAIL')
    ...
    if (role !== 'ADMIN' && !staffName && role !== 'STUDENT') throw appError('INVALID_INPUT')
    ...
    if (role !== 'ADMIN' && role !== 'STUDENT') await ensureAvailable(admin, 'staff', 'staff_code', staffCode, 'STAFF_CODE_ALREADY_EXISTS')
    if (role === 'STUDENT' && studentCodeInput) await ensureAvailable(admin, 'students', 'student_code', studentCodeInput, 'STUDENT_CODE_ALREADY_EXISTS')
    ...
    const profile = await admin.from('profiles').insert({ user_id: createdUserId, role, username, display_name: displayName, email, phone: phone || null, created_by: caller.profile.id, force_password_change: false }).select('id,user_id,role,username,display_name').single()
    ...
    if (role === 'STUDENT') {
      ...
      const student = await admin.from('students').insert({ user_id: createdUserId, student_code: studentCode, full_name: studentName, parent_name: valueFrom(body.student, 'parent_name') || null, parent_phone: valueFrom(body.student, 'parent_phone') || null, email, phone: phone || null, created_by: caller.user.id }).select('id,student_code').single()
      if (student.error || !student.data) throw student.error || new Error('STUDENT_CREATE_FAILED')
    } else if (role !== 'ADMIN') {
      stage = 'staff.insert'
      const staff = await admin.from('staff').insert({ user_id: createdUserId, staff_type: role, staff_code: staffCode || null, full_name: staffName, email, phone: phone || null, created_by: caller.user.id }).select('id,staff_code').single()
      if (staff.error || !staff.data) throw staff.error || new Error('STAFF_CREATE_FAILED')
    }
    stage = 'audit.write'
    const audit = await admin.rpc('write_audit', { p_actor_user_id: caller.user.id, p_action: 'ACCOUNT_CREATE', p_entity_type: 'profiles', p_entity_id: profile.data.id, p_new_data: { role, username } })
    if (audit.error) throw audit.error
    return ok({ profile: profile.data, temporary_password: password }, requestTraceId)
  } catch (error) {
    if (createdUserId && admin) {
      try {
        ...
        return fromError(rollbackError, requestTraceId, { stage: 'rollback', role })
      }
    }
