...
export function fail(
  ...
  return new Response(
    JSON.stringify({ success: false, error: { code, message }, trace_id: id }),
    {
      status,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    },
...
function classifyDatabaseConflict(errorText: string): string | null {
  if (
    !errorText.includes("23505") && !errorText.includes("duplicate") &&
    !errorText.includes("unique")
  ) return null;
  if (
    errorText.includes("profiles_username_lower_uidx") ||
    errorText.includes("profiles_user_id_key") ||
    errorText.includes("(username)")
  ) return "USERNAME_ALREADY_EXISTS";
  if (
    errorText.includes("profiles_email_lower_uidx") ||
    errorText.includes("(email)")
  ) return "EMAIL_ALREADY_EXISTS";
  if (
    errorText.includes("profiles_phone_uidx") || errorText.includes("(phone)")
  ) return "PHONE_ALREADY_EXISTS";
  if (
    errorText.includes("staff_staff_code_key") ||
    errorText.includes("(staff_code)")
  ) return "STAFF_CODE_ALREADY_EXISTS";
  if (errorText.includes("students_student_code_key")) {
...
function classifyError(
...
): { code: string; message: string; rawCode: string; status: number } {
  const value = asErrorLike(error);
  const message = error instanceof Error
    ? error.message
    : textValue(value.message) || "Đã xảy ra lỗi không xác định";
  const explicitCode = textValue(value.code);
  const status = typeof value.status === "number" ? value.status : 0;
  const combined = `${explicitCode} ${message} ${textValue(value.details)}`
    .toLowerCase();

  if (knownCodes.includes(explicitCode)) {
    return { code: explicitCode, message, rawCode: explicitCode, status };
  }
  if (knownCodes.includes(message)) {
    return { code: message, message, rawCode: explicitCode || message, status };
  }

  const conflictCode = classifyDatabaseConflict(combined);
  if (conflictCode) {
    return { code: conflictCode, message, rawCode: explicitCode, status };
  }
  ...
  if (
    ...
    return {
      ...
      message,
      rawCode: explicitCode,
      status,
    };
  }
  if (context.stage === "auth.create") {
    return {
  ...
  if (context.stage === "profiles.insert") {
    return {
  ...
  if (context.stage === "students.insert") {
    return {
...
export function fromError(
  ...
  return fail(
    classified.code,
    publicMessages[classified.code] ||
      (knownCodes.includes(classified.code)
        ? classified.message
