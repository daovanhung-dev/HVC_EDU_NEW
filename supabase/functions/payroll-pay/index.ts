...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const result = await admin.rpc('pay_payroll', { p_payroll_period_id: body.payroll_period_id, p_actor_user_id: caller.user.id })
    if (result.error) throw new Error(result.error.message)
    const staff = await admin.from('payroll_items').select('staff(user_id)').eq('payroll_period_id', body.payroll_period_id)
    await notifyUsers(admin, (staff.data || []).map((row: any) => row.staff?.user_id), 'Lương đã được thanh toán', 'Bảng lương của bạn đã được thanh toán.', { payroll_period_id: body.payroll_period_id })
