...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const inserted = await admin.from('profiles').insert({ user_id: created.data.user.id, role: 'ROOT_ADMIN', username, display_name: 'Admin Root', email, force_password_change: false }).select('id,user_id,role,username').single()
    if (inserted.error) {
