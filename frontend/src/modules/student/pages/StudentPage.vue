...
<template>
  ...
  <div class="card border-0 shadow-sm"><div class="card-body">
    ...
    <div v-else class="table-responsive"><table class="table align-middle"><thead><tr><th>Thời gian</th><th>Lớp</th><th>Phòng</th><th>Trạng thái</th><th>Ghi chú</th></tr></thead><tbody><tr v-for="row in sessions" :key="row.id"><td>{{ formatDateTime(row.scheduled_start_at) }}</td><td>{{ row.class_months?.classes?.name || '—' }}</td><td>{{ row.class_month_schedules?.room ? `Phòng ${row.class_month_schedules.room}` : '—' }}</td><td><span class="badge" :class="row.status === 'COMPLETED' ? 'text-bg-success' : row.status === 'CANCELLED' ? 'text-bg-danger' : 'text-bg-secondary'">{{ row.status }}</span></td><td>{{ row.status === 'CANCELLED' ? 'Buổi học đã hủy' : row.status === 'COMPLETED' ? 'Đã hoàn thành' : 'Theo lịch trung tâm' }}</td></tr><tr v-if="!loading && !sessions.length"><td colspan="5" class="text-center text-secondary py-4">Chưa có lịch học</td></tr></tbody></table></div>
  </div></div>
</template>
